...
const AssignedJobs = () => {
  ...
  const { getOneJob, getVerifiedLawyers, assignJob, verifyLawyer } =
    adminRoute();
  ...
  useEffect(() => {
    if (statusFilter === "Verified Lawyers") {
      ...
      getOneJob(setMessage, setLoading, setIsSuccessful, jobId, setSelectedJob);
    }
  }, [statusFilter]);
  ...
  useEffect(() => {
    if (
      statusFilter === "Applied Lawyers" &&
      selectedJob &&
      selectedJob?.appliedLawer
    ) {
  ...
  const handleAssignClick = () => {
    ...
    if (selectedLawyer?.verified === "false") {
      ...
      assignJob(body, setMessage, setLoading, setIsSuccessful);
    }
  };
    ...
    <AdminNavbar>
      <div className="py-5 my-3 px-2 px-sm-3">
        <div className="justify-content-center mx-5 px-lg-5 mb-5">
          <div
            className="card gap-3 p-3 p-sm-5"
            style={{
              border: "none",
        ...
        <div className=" card p-3">
          <div
            className="d-flex gap-5 py-3"
            style={{ borderBottom: "1px solid #CFCFCF" }}
          >
            <button
              onClick={() => handleStatusFilterChange("Applied Lawyers")}
              className={
                statusFilter === "Applied Lawyers"
                  ? "verify-active me-md-5"
                  : "verify-button me-md-5"
              }
            >
              Applied Lawyers
            </button>
            <button
              onClick={() => handleStatusFilterChange("Verified Lawyers")}
              className={
                statusFilter === "Verified Lawyers"
                  ? "verify-active "
                  : "verify-button "
              }
            >
      ...
      <ViewMoreModal
        handleViewMoreClose={handleViewMoreClose}
        handleViewMoreShow={handleViewMoreShow}
        showViewMoreModal={showViewMoreModal}
        lawyer={selectedLawyer}
        handleVerify={handleVerify}
        handleLawyerProfileNavigation={handleLawyerProfileNavigation}
      />
    </AdminNavbar>
  );
};
